...
/**
 * CHANGED: Renamed class from OAuthClient to F1_Oauth to follow KO3 naming convention
 * Also renamed file name to f1_oauth to follow KO3 convention
 */
class F1_Oauth {
    ...
    private function init_curl() {
        ...
        /**
         * CHANGED: curl_setopt( $this->connection, CURLINFO_HEADER, false );
		 * to: curl_setopt( $this->connection, CURLOPT_HEADER, false ); it was written wrong in original code
         */
    ...
    public function setPathsFromConfig() {
		...
		/**
		 * CHANGED: Load config from KO3 config file as opposed to app config
		 */
        // Load from kohana config
		$this->config = Kohana::config('f1');
		
        $this->requesttoken_path	= $this->config['requesttoken_path'];
        $this->accesstoken_path	= $this->config['accesstoken_path'];
        $this->auth_path = $this->config['auth_path'];
        $this->callbackUrl = $this->config['callbackURI'];
    }
    /************* END- Path Setter **********************/
    ...
    private function sendRequest( $httpMethod, $requestURL, $nonOAuthHeader = array(), $requestBody = "", $successHttpCode = 201 ) {
		...
		/**
		 * CHANGED: commented out below since we are not using appconfig file
		 */
    ...
    */
	...
	/**
	 * CHANGED: mktime() to time() do to Kohana strict coding
	 */
    ...
    private function logRequest($responseBody, $requestBody = "", $transferInfo = array()) {
		...
		/**
		 * CHANGED: commented out since we are not using appconfig file and below is just for debug
		 */
		/*
        if(AppConfig::$debug){
            print $this->lineBreak."[--------------------BEGIN DEBUG----------------------------]".$this->lineBreak;
